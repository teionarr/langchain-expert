...
5. **Qualify each candidate before investing** — three cheap checks that prevent most wasted effort and most over-calls:
   - **Reachable?** Trace it to a *default* config value and a live call path. A finding behind an off-by-default flag, or in a function whose only writers are dead code, is not the severity it looks like. (Check the default constant, then `grep` the callers.)
   - **Already known?** Search the repo's issues *and* open PRs. A great bug with a stalled fix PR needs no second report.
   - **Maintained?** Check last-merge activity. An abandoned repo is not worth a report.
6. **Report ≤7 findings**, severity by rule: 🔴 = legacy-era APIs, a security control that doesn't do what it claims, unbounded spend, or a silent-corruption bug. 🟠 = bites under load or over time. 🟡 = rest. Never manufacture findings — "two findings and a long N/A list" is a valid review.
