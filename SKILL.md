...
4. **Walk the practices**, weighted by the value table below. Expect most to be N/A on mature code and most to fire on early code — the practices are a floor, not a ranking, so never skip the "basic" ones because a repo looks sophisticated. A bug you notice in a file you opened only for *context* is **not a finding in this review** — note it in one line or drop it; a review that chases every thread it pulls loses the plot and pads the report.
...
**B. Models**
5. Models are strings in config (`init_chat_model("provider:model")`); best form is role → alias → gateway, so operators re-point models without a deploy. A provider **instance** for custom settings (`create_agent(model=ChatAnthropic(temperature=0))`) at the agent-construction/wiring site is fine — the framework recommends it, so don't flag it. The finding is a provider class buried in *business logic* or request handling, where swapping a model needs a code change and redeploy.
...
19. Tests use deterministic fakes and assert on *trajectory*, not prose. Custom integrations run `langchain-tests`. Live-API calls in unit tests are a finding. *Not a finding:* a live call in a file explicitly marked integration/e2e or behind a network-gated marker — that is its correct home; confirm the marker before flagging.
