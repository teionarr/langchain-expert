...
**Find it by asymmetry, not pattern-matching.** Grepping for `.where(id == ...)` mostly yields false positives, because the check normally sits one frame up in a helper. Instead: locate the module's canonical guard (`check_*_access`, `_resolve_visible_*`), enumerate its call sites, and diff against every handler touching that resource — **the outliers are the finding**. Two high-yield tells: an identity parameter that is accepted, logged, or even stored but **never appears in a query predicate**; and a guard whose test file has a `rejects_*` case for every sibling but one.
